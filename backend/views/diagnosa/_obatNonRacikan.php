...
<div class="row">
    <?php $form = ActiveForm::begin([
        ...
        'action' => ['diagnosa/saveNonRacikan?id='.$_GET['id'].'&param=Kerja'],
        'formConfig' => ['labelSpan' => 1, 'spanSize' => ActiveForm::SIZE_SMALL,'showLabels'=>false]
    ]);
    ?>
    <?php if($modelResepNonRacikan == null): ?>
    ...
    <div class="col-md-6">
        <div class="form-group">
            ...
            <div class="col-md-3">
                <?= $form->field($modelResepNonRacikan, 'no_resep')->textInput(['readonly' => true]) ?>
            </div>
        </div>
        <div class="form-group">
            <label class="control-label col-md-4">Dokter</label>
            <div class="col-md-3">
                <input type="text"  class="form-control" readonly="true" value="<?= $modelResepNonRacikan->user->username ?>">
            </div>
        </div>
    </div>
    <div class="col-md-6">
        <?php $list = ['CITO!' => 'CITO!','Statim' => 'Statim','Urgent' => 'Urgent','P.I.M' => 'P.I.M'] ?>
        <?= $form->field($modelResepNonRacikan, 'status')->dropDownList($list) ?>
    </div>

    <div class="col-md-12">
        <a class="btn btn-info" id="tambah_obat">Tambah Obat</a>
        <div id="nama_obat">
            <?php if($modelResepNonracikanDetailIsi != null): ?>
                <?php
                    foreach ($modelResepNonracikanDetailIsi as $value):

                ?>
                    <div class="form-group">
                        <label class="control-label col-md-1"><b>R/</b></label>
                        <div class="col-md-2">
                            <input type="text" class="form-control" readonly value="<?= $value['nama_obat'] ?>">
                        </div>
                        <div class="col-md-2">
                            <input type="text" class="form-control" readonly value="<?= $value['kek_isi'] ?>">
                        </div>
                        <div class="col-md-2">
                            <input type="text" class="form-control" readonly value="<?= $value['sediaan'] ?>">
                        </div>
                        <div class=col-md-2>
    ...
    <input type="hidden" name="id_resep_nonracikan" value="<?= $modelResepNonRacikan['id']; ?>">
    <div class="form-group">
        <label class="control-label col-md-2">iter :</label>
        <div class="col-md-2">
            <?php
                $listIter = ['n.i' => 'n.i', '1x' => '1x', '2x' => '2x', '3x' => '3x', '4x' => '4x'];
                echo $form->field($modelResepNonRacikan, 'iter')->dropDownList($listIter);
            ?>
        </div>
    </div>
    <div class="form-group">
        <label class="control-label col-md-2">&nbsp;</label>
        <div class="col-md-2">
            <?= $form->field($modelResepNonRacikan, 'label_etiket')->checkbox(['label' => 'Label/Etiket']) ?>
        </div>
    </div>
    <div class="form-group">
        <label class="control-label col-md-2">&nbsp;</label>
