...
<div class="row">
    ...
    ?>
    <?php if($resepNonracikanIsi == null){ ?>
    ...
    <div class="col-md-6">
        <div class="form-group">
            ...
            <div class="col-md-3">
                <?= $form->field($resepNonracikanIsi, 'no_resep')->textInput(['readonly' => true]) ?>
            </div>
        </div>
        <div class="form-group">
            <label class="control-label col-md-4">Dokter</label>
            <div class="col-md-3">
                <input type="text"  class="form-control" readonly="true" value="<?= $resepNonracikanIsi->user->username ?>">
            </div>
        </div>
    </div>
    <?php } ?>
    <div class="col-md-6">
        ...
        <?= $form->field($resepNonracikanIsi, 'status')->dropDownList($list) ?>
    </div>

    <div class="col-md-12">
        <a class="btn btn-info" id="tambah_obat">Tambah Obat</a>
        <div id="nama_obat">
            <?php if($resepNonracikanDetailIsi != null): ?>
                <?php
                    foreach ($resepNonracikanDetailIsi as $value):
    ...
    <input type="hidden" name="id_resep_nonracikan" value="<?= $resepNonracikanIsi['id']; ?>">
    <div class="form-group">
        <label class="control-label col-md-2">iter :</label>
        <div class="col-md-2">
            <?php
                $listIter = ['n.i' => 'n.i', '1x' => '1x', '2x' => '2x', '3x' => '3x', '4x' => '4x'];
                echo $form->field($resepNonracikanIsi, 'iter')->dropDownList($listIter);
            ?>
        </div>
    </div>
    <div class="form-group">
        <label class="control-label col-md-2">&nbsp;</label>
        <div class="col-md-2">
            <?= $form->field($resepNonracikanIsi, 'label_etiket')->checkbox(['label' => 'Label/Etiket']) ?>
        </div>
    </div>
    <div class="form-group">
        <label class="control-label col-md-2">&nbsp;</label>
        ...
        $('#simpan-obat').click(function(){
            $.ajax({
                ...
                success:function(data){
                    ...
                    location.reload(baseurl + '/saran-anjuran/index?id=<?php echo $_GET['id']; ?>#w1-tab1');
                }
            });
        });
    });
